...
//            @NotBlank(message = "{required}") String verifyCode,
            boolean rememberMe, HttpServletRequest request) throws AlphaException {
...
//        password = MD5Util.encrypt(username.toLowerCase(), password);
    ...
    public Response regist(
            @NotBlank(message = "{required}") String username,
            @NotBlank(message = "{required}") String password) throws AlphaException {
        User user = userService.findByName(username);
        if (user != null) {
            throw new AlphaException("该用户名已存在");
        }
    ...
    public void captcha(HttpServletRequest request, HttpServletResponse response) throws IOException, AlphaException {
