...
public class SortUtil {
    ...
    public static void handlePageSort(QueryRequest request, Page<?> page, String defaultSort, String defaultOrder, boolean camelToUnderscore) {
        ...
        if (camelToUnderscore) {
            sortField = AlphaUtil.camelToUnderscore(sortField);
            defaultSort = AlphaUtil.camelToUnderscore(defaultSort);
        }
        if (StringUtils.isNotBlank(request.getField())
            ...
            if (StringUtils.equals(request.getOrder(), AlphaConstant.ORDER_DESC))
                page.addOrder(OrderItem.desc(sortField));
            else
                page.addOrder(OrderItem.asc(sortField));
        } else {
            if (StringUtils.isNotBlank(defaultSort)) {
                if (StringUtils.equals(defaultOrder, AlphaConstant.ORDER_DESC))
