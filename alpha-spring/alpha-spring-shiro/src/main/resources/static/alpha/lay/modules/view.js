    ...
    .define(
        ...
        function (exports) {
            ...
            self.loading = function (elem) {
                elem.append(
                    (this.elemLoad = $(
                        '<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading alpha-loading"></i>'
                    ))
                )
            };
            ...
            self.modal.base = function (msg, params) {
                ...
                params = $.extend(
                    {
                        skin: 'layui-layer-admin-modal alpha-alert',
            ...
            self.tab = {
                ...
                minLeft: null,
                maxLeft: null,
                wrap: '.alpha-tabs-wrap',
                menu: '.alpha-tabs-menu',
                next: '.alpha-tabs-next',
                prev: '.alpha-tabs-prev',
                step: 200,
                init: function () {
                    var tab = this;
                    var btnCls = tab.wrap + ' .alpha-tabs-btn';

                    layui.dropdown.render({
                        elem: '.alpha-tabs-down',
                        click: function (name) {
                            if (name === 'all') {
                                tab.delAll();
                            }
                            ...
                            if (name === 'current') {
                                tab.del(layui.alpha.route.fileurl);
                            }
                            if (name === 'refresh') {
                                tab.refresh();
                            }
                    ...
                    $(document).on('click', btnCls, function (e) {
                        ...
                        if ($(e.target).hasClass('alpha-tabs-close')) {
                            tab.del(url)
                        } else {
                    ...
                    $('.alpha-tabs-hidden').addClass('layui-show');
                    this.isInit = true
                },
                has: function (url) {
                    var exists = false;
                ...
                delAll: function (type) {
                    var tab = this;
                    var menuBtnClas = tab.menu + ' .alpha-tabs-btn';
                ...
                del: function (url, backgroundDel) {
                    ...
                    var lay = '[lay-url="' + url + '"]';
                    var thisBody = $(
                        '#' + conf.containerBody + ' > .alpha-tabs-item' + lay
                    );
                    ...
                    if (backgroundDel === undefined) {
                        if (thisMenu.hasClass('alpha-tabs-active')) {
                            $(this.menu + ' li:last').click()
                        }
                    }
                },
                refresh: function (url) {
                    url = url || layui.alpha.route.fileurl;
                    if (this.has(url)) {
                        this.del(url, true);
                        self.renderTabs(url)
                    }
                ...
                clear: function () {
                    this.data = [];
                    this.isInit = false;
                    $(document).off('click', this.wrap + ' .alpha-tabs-btn')
                },
                change: function (route, callback) {
                    ...
                    var activeCls = 'alpha-tabs-active';
                    ...
                    if (existsTab) {
                        ...
                        layui.alpha.navigate(route.href)
                    } else {
                        self.loadHtml(fileurl, function (res) {
                            var htmlElem = $(
                                "<div><div class='alpha-tabs-item' lay-url='" +
                                fileurl +
                                "'>" +
                                res.html +
                                '</div></div>'
                            ...
                            tab.data.push(route);
                            layui.alpha.render(tab.tabMenuTplId);
                    ...
                    layui.alpha.sidebarFocus(route.href);
                    return false
                },
                onChange: function () {
                }
            ...
            self.renderLayout = function (callback, url) {
                ...
                self.render(url, function (res) {
                    ...
                    if (conf.viewTabs === true) {
                        self.containerBody.addClass('alpha-tabs-body')
                    }
                    layui.alpha.appBody = self.containerBody;
